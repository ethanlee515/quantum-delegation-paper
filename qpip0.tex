...
\end{theorem}
\begin{proof}
	There's a \emph{dummy strategy} for $\PiNaive$'s prover, who chooses $\rho$ as maximally mixed state instead, and executes the rest of the protocol honestly.
	It is accepted in the testing round with probability $1-\negl(\lambda)$,
	but has negligible probability passing the verification after the Hadamard round.

	\Ethan{Smooth this out}
	...
	First, for the case that $\PNaiveStar$ aborts, since dummy strategy will be rejected with overwhelming probability in Hadamard round,
...
We begin by considering the state $\ket{\psi}$ the prover in $\PiSampZ$ holds before he receives $\vec{c}$. We denote the corresponding Hilbert space as $H_{\regX,\regZ}$.
...
\begin{proof}
	...
	Denote the result we would get when $c=1$ by $z_{composite,i}$. By construction, when $G_{i,1,\gamma}G_{i-1,1,\gamma} \cdots G_{2,1,\gamma}G_{1,1,\gamma}$ succeeded, the corresponding output would be $z_{good,i}$. Also note that this is the only case where the verifier won't reject, so $z_{composite,i}=z_{good,i}$.
...
Notice that $z_{ideal}$ is some ideal output distribution, which (a priori) might not have the same accept probability as $z$.
