...
\begin{theorem}
	...
	$\tr\abs{z_{good, i}-z_{ideal, i}}\leq O(\varepsilon)$ \Ethan{Check thm statement again later}
\end{theorem}
\begin{proof}
...
Now we try to put together all $i\in [m]$. Define the following quantities:
...
Notice that $z_{ideal}=pz_{true}$ for some probability $p\in[0, 1]$.

\begin{theorem}
	$\PiSampZ$ is a computationally sound protocol for $\SampBQP$.
\end{theorem}
\begin{proof}
