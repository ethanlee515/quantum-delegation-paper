...
\Ethan{Maybe use norm instead of braket for that acc probability?}
...
Now we define
\Ethan{Need intuitive high-level description on what this is, maybe the good terms of partition or something?}
\begin{align}
...
\begin{proof}
	...
	\begin{align}
		z_i=& z_{good,i}+ \E_\gamma \sum_z \proj{z} \L[\sum_{k=0}^{i-1} \bra{\psi_{1^k0,\gamma}}U^\dag  P_{acc,i,z}U   \sum_{j=0}^{i-1} \ket{\psi_{1^j0,\gamma}}+
	...
	\end{align}
	by expanding out \Ethan{TODO Some expression}, where we omitted writing out $e_i$ starting the second line.
	Therefore we have
	\begin{align} \label{eq:zi-zgoodi}
		\tr|z_i-z_{good,i}|\leq&  \sum_z   \E_\gamma \L[ \sum_{k=0}^{i-1} \sum_{j=0}^{i-1} \L| \bra{\psi_{1^k0,\gamma}}U^\dag  P_{acc,i,z}U \ket{\psi_{1^j0,\gamma}} \R|+
		...
		+&\L.2 \sum_{k=0}^{i-1} \L|\bra{e_i,\psi_{1^k0,\gamma}}U^\dag  P_{acc,i,z}U \ket{e_i,\psi_{1^i,\gamma}} \R|\R] + O\L(\frac{m^2}{\sqrt T}\R)\nn \\ %%%%%%%%%
...
Now define $z_{true}$ to be sampled from $D_x$... \Ethan{Or something like that},
and define $z_{ideal, i}=\tr(z_{good, i})z_{true}$.

\begin{theorem}
	\Ethan{Second part of main theorem this section}
	$z_{good, i}\approx z_{ideal, i}$
\end{theorem}
\begin{proof}
