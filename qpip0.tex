...
\begin{enumerate}
    ...
    \item Both verifier and prover start with a soundness parameter $\varepsilon$.
    \item Run $N=O(1/\varepsilon)$ copies of [[urmilla protocol]] of in parallel.  On the testing round, the verifier uniformly randomly pick one copy and Hadamard round and has all other copies as test round.
    \item Verified reject if any of the test round result is wrong. Otherwise, decode the Hadamard run desult and run \cref{ProtoQPIP1} on it
    . 
\end{enumerate}
\end{protocol}
