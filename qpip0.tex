...
In this section, we combine the core protocol from \cite{FOCS:Mahadev18a}, which we restate as Protocol~\ref{proto:urmila4} \Ethan{Just say $\PiMeasure$}, with \Cref{QPIP1thm} to create a delegation protocol for $\SampBQP$ for fully classical clients. A direct composition results in Protocol~\ref{proto:qpip0_naive}. However, as we will see in \Cref{sec:urmila4}, Protocol~\ref{proto:urmila4} has a peculiar and weak guarantee, so \Cref{proto:qpip0_naive} don't have any reasonable completeness and soundness. To boost its soundness, we run $m=\poly(\lambda)$ copies in parallel and test on $m-1$ copies of them, resulting in Protocol~\ref{proto:QPIP0samp}. We use techniques from \cite{arXiv:ChiaChungYam19} to prove that Protocol~\ref{proto:QPIP0samp} has soundness error \Ethan{Phrase in terms of accuracy param instead} $1/\sqrt{m}$.

\Ethan{Go even more high level. This roadmap is confusing to people who aren't familiar with the relevant literature}

\subsection{Mahadev's measurement protocol}\label{sec:urmila4}

In her groundbreaking work, Mahadev gives a $\QPIP_0$ protocol for $\BQP$ languages.
The core of this work is a 4-round $\QPIP_0$ protocol $\PiMeasure$ that allow a $\BQP$ prover to ``commit a state" for a classical verifier to choose a measurement basis and obtain corresponding measurement results.
One can see $\cVGS$ as a $\QPIP_1$ protocol where the prover sends copies of ground state to the verifier to measure;
which can then be transformed to a $\QPIP_0$ protocol by replacing the verifier's measurement with calls to $\PiMeasure$.

The details of $\PiMeasure$ is as follows. The verifier chooses a string $h$ specifying the measurements he wants to make, and generates keys $pk, sk$ from $h$. It sends $pk$ to the prover. The prover ``commits" to a state $\rho$ of its choice using $pk$ and replies with its commitment. \Ethan{Need more intuition} The verifier then uniformly chooses between two options: do a \emph{testing round} or do a \emph{Hadamard round}. He sends his choice to the prover, and prover replies accordingly. If the verifier chose testing round, he checks the prover's reply against the previous commitment, and rejects if he sees an inconsistency. If the verifier chose the Hadamard round, he calculates $M_{XZ}(\rho, h)$ based on the reply. \cite{FOCS:Mahadev18a} shows a ``binding" property of $\PiMeasure$: if a prover would always succeed on the testing round, then there exists some $\rho$ so that the verifier obtains $M_{XZ}(\rho, h)$ if he had chosen Hadamard round, no matter what $h$ the verifier has picked at the beginning.
...
\begin{lemma}[binding property of $\PiMeasure$]
	...
	Let $\PMeasureStar$ be a $\BQP$ cheating prover for $\PiMeasure$ and $\lambda$ be the security parameter. Under the QLWE assumption, if $\PMeasureStar$ passes the testing round with probability \Ethan{Not precise enough - probability over what? For all $h$ or $pk$?} $1-\negl(\lambda)$, then there exists some $\rho$ so that for all verifier's input $h \in \zo^n$, the verifier's outputs on the Hadamard round is computationally indistinguishable from $M_{XZ}(\rho, h)$.
	\iffalse    
...
Namely, if a cheating prover passes the testing round with overwhelming probability conditioned on it doesn't ``give up" on the 2nd message, then... \Ethan{Word this better}
...
otherwise commits with the maximally mixed states.\Ethan{Finish this high level proof sketch}
...in order to use \Cref{lem:urmila-binding}.
...
\begin{theorem}[binding property of $\PiNaive$]
	...
	Under the QLWE assumption, \Ethan{Add ``if P satisfies that..."} if conditioned on $\PNaiveStar$ not aborting (ie. $y\ne\bot$) \Ethan{Make even more explicit ie. say not aborting on round 2}, $\PNaiveStar$ passes the testing round with probability $1-\negl(\lambda)$,
	then the verifier's output in the Hadamard round is $O(\eps)$-computationally indistinguishable from $(d, z_{ideal})$. \Ethan{Check if this notation is defined; give reminder even if it is}
\end{theorem}
\begin{proof}
...
To begin, for all $i\in[m]$, we decompose $\ket{\psi}$ as follows: \Ethan{Paragraph break before this; should define everything earlier again}
...
\Ethan{TODO explain more pointwise vs average argument w.r.t. $pk, y$}
...
Now we define
\Ethan{TODO what is this? Need intuitive idea.}
\begin{align}
