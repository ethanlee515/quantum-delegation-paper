...
\label{proto:urmila4}
\Ethan{TODO Make notation consistent with protocols in other sections}
...
% \begin{lem}[another triangle inequality for computational distance \hannote{unused}]\label{lem:computational-triangle}
...
By combining the commitment Protocol, Protocol~\ref{proto:urmila4}, and the $\QPIP_1$ Protocol, Protocol~\ref{ProtoQPIP1}, we get a naive $\QPIP_0$ Protocol for $\SampBQP$ that has no soundness guarantee.

\Ethan{Probably can omit details from below}

\begin{protocol}{naive $\QPIP_0$ protocol  for the $\SampBQP$ instance $(D_x)_{x\in\set{0, 1}^*}$}
	\label{proto:qpip0_naive}

	Inputs: \Ethan{Just assume PiSamp has the properties I need including non-adaptive measurements. Maybe make author note for now if needed and add it back later}
	\begin{itemize}
...
%  		\item Let $\cVGS$ be the way to verify said ground state as defined in \Cref{AlgGroundStateCheck}. \Ethan{TODO wording needs fix...? How?}
	...
	\item \label{step:urmila-in-naive} The verifier and prover run Protocol~\ref{proto:urmila4}, with the following parameters: $h$ is chosen from previous step, $n_{commit}=M \, s$ , where $n_{commit}$ is the $n$ for Protocol~\ref{proto:urmila4}, and the honest prover choose $M$ copies of $\ket{\psi_{C'(x)}}$ as $\rho$. We also allow a cheating prover to send $\bot$ in the fourth round of Protocol~\ref{proto:urmila4} if $c=1$ to abort the process.\footnote{This is useless for the cheating prover but useful for proving one of our theorems.} If the verifier receives $\bot$ in the fourth round, he outputs $(\Rej,\bot)$ and terminates this protocol. Let $z_{commit}$ be the output of the called Protocol~\ref{proto:urmila4}.
	
	\item If $c=1$, the verifier finished the verification step of Protocol~\ref{ProtoQPIP1} by using $z_{commit}$  as the measurement result he would get and outputs $(d,z_{naive})$.
...
%    \item Verifier start with description of quantum circuit $C$, input string $x$ 
    ...
    \item Run $m=O(1/\varepsilon^2)$ copies of protocol~\ref{proto:urmila4} in parallel \Ethan{proto 2? Or 3?}, with following modifications. The copies have separate $\{h\}$, $\{sk\}$, $\{pk\}$, $\{y\}$, $\{c\}$, and $\{a\}$.  The verifier draws $\{h\}$ independently from the distribution specifier by step~\ref{step:qpip1-verify} of Protocol~\ref{ProtoQPIP1} to generate $\{sk\}$ and $\{pk\}$ for all copies on the first round. On the second round, for each copy, the prover choose the state specified by step~\ref{step:qpip1-state-gen} of Protocol~\ref{ProtoQPIP1}  (with  $(C,x,\eps)$ as input) as $\rho$. On the third round, the verifier uniformly randomly pick an $i\in [m]$, do a Hadamard round on $i$-th copy, and has do test rounds on all other copies, i.e. $c_i=1$ and $c_j=0$ for all $j\neq i$.
