...
\begin{proof}
	...
	If $y=\bot$ and $c=0$, continue the dummy strategy.

	Observe that this $\Pstar$ indeed passes testing round with overwhelming probability by construction.
	So we can apply \Cref{lem:urmila-binding} to the $\PiMeasure$ call to use its binding property.
	Combining it with $\PiSamp$'s soundness,
	we see that the verifier's outputs corresponding $\Pstar$, $(d', z')$ is computationally indistinguishable to some $(d', z_{ideal}')$

	Conditioned on $\PNaiveStar$ accept, clearly $(d, z)=(d', z')$.
	On $\PNaiveStar$ reject, since dummy strategy will be rejected with overwhelming probability in Hadamard round, \Ethan{kinda obvious but we never explicitly explained why}
	we also have $(d, z)=(\Rej, \bot)\approx_{c, \negl(\lambda)}(d', z')$

	The conclusion follows from a simple hybrid argument.
\end{proof}
...
Let $(U_0,U)$ be a prover's strategy in a $m$-fold parallel repetition of $\PiMeasure$, where $U_0$ is how the prover generates $\vec{y}$ on the second message, and $U$ is how the prover generates $\vec{a}$ on the fourth message. Denote the string $0^{i-1}10^{m-i} \in \zo^m $ as $e_i$, which corresponds to Hadamard round on the $i$-th copy and testing round on all others. Let $\gamma_0 \in[0,1]$, and $T\in \mathbb{N}$ such that $\gamma_0=1/\poly(n)$ and $T=\poly(n)$. \Ethan{These $n$ probably should be $\lambda$}
...
% $$ \ket{\psi}_{\regX,\regZ} =  G_{0,i,\gamma}\ket{\psi}_{\regX,\regZ}+ G_{1,i,\gamma}\ket{\psi}_{\regX,\regZ}+\ket{\psi_{err}}_{\regX,\regZ},$$
  ...
  Note that $G_{0,i,\gamma}$ and $G_{1,i,\gamma}$ has failure probabilities, and this is reflected by the fact that $\ket{\psi_{0,i,\gamma}}_{\regX,\regZ}$ and $\ket{\psi_{1,i,\gamma}}_{\regX,\regZ}$ are  sub-normalized. $G_{0,i,\gamma}$ and $G_{1,i,\gamma}$ depend on $(U_0,U)$ and $\vec{pk},\vec{y}$. \hannote{check}
