...
\begin{lem}
	...
	Then, there exists an efficient quantum procedure $G_{i,\gamma}$ such that for any (possibly sub-normalized) quantum state $\ket{\phi}_{X, Z}$,

	$$TODO-LONG-EQ$$

	where $t$ is the number of qubits in the register $ph$, $z_0, z_1\in\bbC$ such that $\abs{z_0}=\abs{z_1}=1$, and $z_0$, $z_1$, $\ket{\phi_0}_{X, Z}$, $\ket{\phi_1}_{X, Z}$, $\ket{\phi_{err}}_{X, Z}$ may depend on $\gamma$.
