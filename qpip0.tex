...
%\footnote{Two distributions has $c$ computational distance if any efficient algorithm can produce at most $c$ bias between them.}
...
There is an additional issue, however. As $G_{1,i}$ does not commute with $G_{1,j}$, $\ket{\psi_{1^m}}$ is unfortunately only binding for the $m$-th copy.
...
Note that in property~\ref{property:partition-binding}, we are using $\ext_i$ instead of $U$ because we are doing some amplitude amplification to boost the success probability. \hannote{some discussion on why we want $G$ to be efficient.}
...
For all $k \in [m]$, $d\in \zo^k$, $\gamma=(\gamma_1, \ldots, \gamma_k)$ where each $\gamma_j\in\set{\frac{\gamma_0}{T},\frac{2\gamma_0}{T},\dots,\frac{T\gamma_0}{T}}$ \Ethan{This ok?}, and $\ket{\psi} \in H_{\regX,\regZ}$, define $$\ket{\psi_{d,\gamma}}\defeq G_{d_k,k,\gamma_k}\ldots G_{d_2,2,\gamma_2} G_{d_1,1,\gamma_1} \ket{\psi}.$$
...
it clearly follows that \Ethan{This ok?}
\begin{rmk}
...
\begin{proof}
	...
	$z_{composite,i}$ is $O(\eps)$-computationally indistinguishable to $z_{ideal, i}$. \hannote{fix notation difference..}
\end{proof}
...
\begin{theorem}\label{thm:qpip0} \hannote{to the form of thm 4.4. call to thm 1.1?}
...
\begin{proof}
	...
	Set $m=O(1/\eps^2), T=O(1/\eps^2),\gamma_0=\eps^8$. Combining \Cref{thm:zgood-zideal} and \Cref{eq:z-z-good} by triangle inequality, we have $\tr\abs{z-z_{ideal}}=O(\varepsilon)$. \hannote{computational?}
