...
It says that for each $i\in[m]$, there exist two efficient ``projectors" \footnote{Actually they are not projectors, but for the simplicity of this discussion let's assume they are.} $G_{0,i}$ and $G_{1,i}$ in the prover's internal space with $G_{0,i}+G_{1,i} \approx Id$.
$G_{0,i}$ represents the subspace where the server knows the answer to the testing round on the $i$-th copy, while $G_{1,i}$ represents the subspace where the server doesn't.
By using this partition lemma iteratively, we can split the prover's internal state $\ket{\psi}$ by the first copy a component will fail on a testing round. 
That is, $\ket{\psi}=\ket{\psi_0}+\ket{\psi_{10}}+\ldots+\ket{\psi_{1^{m-1}0}}+\ket{\psi_{1^m}}$, where $\ket{\psi_{1^i0}}$ represents the component that would pass the first $i$ testing rounds then gets rejected by the next one, and $\ket{\psi_{1^m}}$ represents the ``good" component where the prover knows the answer to every testing round.
% As each $\ket{\psi_{1^i0}}$ survives only when the Hadamard round choice is $c=e_{i+1}$, that is with low probability, the output corresponding to $\ket{\psi}$ would be close to that of the subnormalized state $\ket{\psi_{1^m}}$.

\Ethan{This paragraph needs more work}
There is an additional issue, however, as $G_{1,i}$ don't commute with $G_{1,j}$, so $\ket{\psi_{1^m, \gamma}}$ is only binding for the $m$-th copy.
To solve this problem, we start with a pointwise argument and fix the Hadamard round on the $i$-th copy,
...
\Ethan{Maybe we can delete a couple of the following equations now, since the intuitive idea was presented earlier?}
