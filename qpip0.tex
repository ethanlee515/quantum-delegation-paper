...
\begin{lemma}\label{lem:urmila-binding}[binding property of Protocol~\ref{proto:urmila4}\cite{FOCS:Mahadev18a}]
	Let $\PMeasureStar$ be a cheating $\BQP$ prover for $\PiMeasure$.
...
\begin{protocol}{Intermediate $\QPIP_0$ protocol $\PiNaive$ for a $\SampBQP$ problem $(D_x)_{x\in\set{0, 1}^*}$}
	...
	Protocol:
	\begin{enumerate}
		\item \label{step:naive1} The verifier choose a $XZ$-measurement $h$ on inputs $1^\lambda$, $1^{1/\varepsilon}$, and $x$ from the distribution specified in \stepref{qpip1-verify} of $\PiSamp$.
		\item \label{step:naive2} The prover prepares $\rho$ on inputs $1^\lambda$, $1^{1/\varepsilon}$, and $x$ by running \stepref{qpip1-state-gen} of $\PiSamp$.
		...
		\item \label{step:naive-output} If it's a Hadamard round, the verifier finishes the verification step of Protocol~\ref{ProtoQPIP1} by generating and outputting $(d, z)$
			on inputs $1^\lambda$, $1^{1/\varepsilon}$, $x$, $h$, and $v$. \Ethan{So many inputs... Should the verifier keep internal states instead?}
	\end{enumerate}
\end{protocol}
...
Protocol:
\begin{enumerate}
	\item The verifier generates $m$ independently copies of basis choices $\vec{h}=(h_1,\ldots,h_m)$ on inputs $1^\lambda, 1^{1/\varepsilon}, x$ as in \stepref{naive1} of $\PiNaive$
