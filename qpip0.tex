...
\Ethan{Write a bit of technical intro. allows prover commit a state and verifier chooses h allows verifier to learn... MXZ introduce here}

\Ethan{Discussion: Intuitively the purpose of this protocol is to let the verifier learn a measurement outcome. Define MXZ rho h notation here.}
...
\begin{itemize}
	...
	\item Verifier's inputs:
		\begin{itemize}
			...
			\item a challenge bit $c\in\zo$. The prover needs to do a ``Testing round" if $c=0$. The prover needs to do a ``Hadamard round" if $c=1$. \Ethan{TODO rephrase. c indicates testing vs Hadamard...}
			\end{itemize}
\end{itemize}

The protocol: \Ethan{Bring back V1 P1 notations}
\begin{enumerate}
    ...
    \item \Ethan{Need more discussion about what's testing/Hadamard} If $c=0$, the verifier \Ethan{performs testing and} decides whether he is going to reject on inputs $(pk,y,a)$.
	If $c=1$, the verifier \Ethan{performs Hadamard round and generates} computes an $n$-bit string $z=V_f(sk,h,y,a)$ on inputs $(sk,h,y,a)$.
	
	\Ethan{Switch to the following wording:
	
	If it's a testing round (c=0), then the verifier...
	If it's a Hadamard round (c=1), then the verifier...}
\end{enumerate}
\end{protocol}

\Ethan{Discussion: Notice that testing round has no outputs, then binding property. Then fact.}

Two modification have been made from the original protocol.
\begin{itemize}
	\item On the protocol's third message. Instead of randomly picking whether to run what's called a ``testing round" or a ``Hadamard round", we let the verifier decide as one of its inputs. \Ethan{Remove this and make $c$ random. Instead of this, say that we do parallel repetition but we change the distribution of $c$}
	\item On the protocol's fourth message, the prover can choose to abort by sending $a=\bot$. This is useless for the cheating prover but useful for proving one of our theorems. \Ethan{Move to later when it's used}
\end{itemize}
...
\begin{lem}[\cite{FOCS:Mahadev18a}\hannote{change to fact?} \Ethan{Ok I'll do that later}]~\label{lem:trivial-4-round-strategy}
    For any input, there exist a prover strategy $(U_{trivial,0},U_{trivial})$ \Ethan{Consider delay introducing the particular notation; just write this name when it's used later} for Protocol~\ref{proto:urmila4} that is accepted with probability $1-\negl{\lambda}$ \Ethan{Changed n to lambda here, see what does it affect} in the testing.
\end{lem}
...
If a prover's strategy in Protocol~\ref{proto:urmila4}\hannote{need clarification} results in passing the testing round with probability $1-\negl(n)$, then there must exist a $\rho$ such that for all $h$, the verifier will produce a $z$ on the Hadamard round that is $\negl(n)$-Q-computationally \Ethan{Remove Q} indistinguishable\footnote{Recall that quantum computational indistinguishability is defined in Definition~\ref{def:comp-indis}.} from $M_{XZ}(\rho,h)$\hannote{add notation}, unless LWE can be efficiently solved by a quantum computer. 
\end{lemma}
...
In particular, we combine them by replacing \Ethan{Still a bit ambiguous. More discussion than just ``replace". Mention it's QPIP1 and whatnot.} the verifier's measurements with a call to $\PiMeasure$. It is possible because $\VSamp$ applies only $X$ and $Z$ measurements non-adaptively.
...
\begin{protocol}{Intermediate $\QPIP_0$ protocol $\PiNaive$ for a $\SampBQP$ problem $(D_x)_{x\in\set{0, 1}^*}$}
	...
	Protocol:
	\begin{enumerate}
		\item The verifier choose a $XZ$-measurement $h$ from the distribution specified in Step~\ref{step:qpip1-verify} of $\PiSamp$. \Ethan{I didn't change the wording here}
		...
		\Ethan{Let z be the verifier output when it's a Hadamard round}
		If $c=1$, the verifier obtains $z_{commit}$ \Ethan{measure/commit... so many notations. Confusing. Just do v}.
		\item If $c=1$, the verifier finishes the verification step of Protocol~\ref{ProtoQPIP1} by using $z_{commit}$ as the measurement result he would get and outputs $(d,z_{naive})$ \Ethan{Just do z here}.
	\end{enumerate}
\end{protocol}

\Ethan{Try to update notation:
Use something else for measurement outcome for proto 3. Maybe v or w.
Let z be sampbqp results.}
