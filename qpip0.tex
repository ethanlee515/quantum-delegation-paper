...
where $\ket{e_i,\psi}=\ket{e_i}_\regC\ket{\psi}_{\regX,\regZ}$ and $U$ is the unitary the prover applies on the last round. \hannote{need to average $pk$ here.}
...
\hannote{$M_{XZ}(\rho_{good},h)=\sum_z \proj{z} \tr\L(\rho \sigma_{h,z} \R)?$}.
...
\end{align}
\hannote{the composition of z ideal and triangle ineq is sketchy lol.}
