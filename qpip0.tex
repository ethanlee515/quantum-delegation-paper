...
\begin{protocol}{Mahadev's measurement protocol $\PiMeasure=(\PMeasure, \VMeasure)$}
	...
	\begin{itemize}
		...
		\item Verifier's input: the measurement basis choice $h \in \zo^n$ \hannote{those two parameters are actually called input?}
	\end{itemize}
...
$\PiMeasure$ has negligible completeness errors, i.e. if both the prover and verifier are honest, the verifier's output on Hadamard round is computationally indistinguishable from $M_{XZ}(\rho, h)$. As for soundness,
...
\begin{lemma}[binding property of $\PiMeasure$; special case of Claim 7.1 in \cite{FOCS:Mahadev18a}]
	...
	Let $\PMeasureStar$ be a $\BQP$ cheating  prover for $\PiMeasure$ and $\lambda$ be the security parameter. Let $1-p_{h,T}$ be the  probability that the verifier accepts $\PMeasureStar$ in the testing round on basis choice $h$.\footnote{Compared to Claim 7.1 of \cite{FOCS:Mahadev18a}, we don't have a $p_{h,H}$ term here. This is because on rejecting a Hadamard round, the verifier can output a uniformly random string, and that is the result of measuring $h$ on the totally mixed state.} Under the QLWE assumption, there exists some $\rho^*$ so that for all verifier's input $h \in \zo^n$, the verifier's outputs on the Hadamard round has $\sqrt{p_{h,T}}+\negl(n) $ computational distance\footnote{Two distributions has $c$ computational distance if any efficient algorithm can produce at most $c$ bias between them.}  from $M_{XZ}(\rho^*, h)$.
...
% \begin{lemma}[binding property of $\PiMeasure$; special case of Claim 7.1 in \cite{FOCS:Mahadev18a}\hannote{correct ref?}]
% 	\label{lem:urmila-binding}
% 	Let $\PMeasureStar$ be a $\BQP$ cheating \hannote{defined?} prover for $\PiMeasure$ and $\lambda$ be the security parameter. Under the QLWE assumption, if $\PMeasureStar$ passes the testing round with probability $1-\negl(\lambda)$ for all $h$\footnote{In terms of \cite{FOCS:Mahadev18a}, we are setting $P_{h,T}$ as a negligible function.}, then there exists some $\rho$ so that for all verifier's input $h \in \zo^n$, the verifier's outputs on the Hadamard round is computationally indistinguishable from $M_{XZ}(\rho, h)$.
% \end{lemma}
...
\begin{protocol}{Intermediate $\QPIP_0$ protocol $\PiNaive$ for a $\SampBQP$ problem $(D_x)_{x\in\set{0, 1}^*}$}
	...
	\begin{enumerate}
		...
		\item \label{step:urmila-in-naive}
			...
			\begin{enumerate}
				\item The verifier samples $(pk, sk)\leftarrow\cVNaive{1}(1^\lambda, h)$ and sends $pk$ to the prover. \hannote{shouldn't those be $V_{meas,1}$?}
				\item The prover runs $(y, \sigma)\leftarrow\cPNaive{2}(pk, \rho)$ and sends $y$ to the verifier.
					Here we allow the prover to abort by sending $y=\bot$, which does not benefit cheating provers but simplifies our analysis.
...
The intuition of the proof of \Cref{lem:naive-qpip0-binding} is to reduce\hannote{not quite?} to \Cref{lem:urmila-binding} by constructing another $\BQP$ prover\hannote{this is really confusing..} $\Pstar$ for $\PiMeasure$. \hannote{and the binding of $P*$ implies the binding of $P^*_{int}$}
...
\begin{theorem}[binding property of $\PiNaive$]
	...
	Suppose that $\Prob{d=\Acc\mid y\ne\bot, c=0}$ is overwhelming for all $x$, \Ethan{This good?}
...
\end{theorem}
\begin{proof}
	We first introduce the \emph{dummy strategy} for ~\Cref{proto:qpip0_naive}, where the prover chooses $\rho$ as the maximally mixed state and executes the rest of the protocol honestly.
   ...
   In the following we fix an $x$.
   
	Now we construct a cheating $\BQP$ prover for $\PiMeasure$, $\Pstar$, that is almost perfect as follows.
...
%That is, there exists some $\rho$ such that $v=M_{XZ}(\rho, h)$.\hannote{only comp}
	...
	Let the distribution on $h$ specified in Step~\ref{step:naive1} of the protocol be $p_x(h)$. By construction $\Pstar$ passes testing round with overwhelming probability over $p_x(h)$, i.e. $\sum_h p_x(h) p_{h,T} =\negl{\lambda}$, where $p_{h,T}$ is $\Pstar$'s probability of getting accepted by the prover on the testing round on basis choice $h$. By \Cref{lem:urmila-binding} and Cauchy's inequality, there exists some $\rho$ such that  $\sum_h p_x(h) \norm{v_h -M_{XZ}(\rho, h)}_c = \negl(\lambda)$, where $\norm{}_c$ means computational distance. \hannote{defined in a footnote ahhhhhhhhh}. Therefore $v= \sum_h p_x(h) v_h$ is computationally indistinguishable to $\sum_h p_x(h) M_{XZ}(\rho, h)$.


    

	Combining it with $\PiSamp$'s soundness (\Cref{QPIP1thm}), \hannote{computational ok?}


	we see that $(d', z')\leftarrow(\Pstar, \VNaive)(1^\lambda, 1^{1/\varepsilon}, x)$ \hannote{fix notation} is $\eps$-computationally indistinguishable to $(d', z_{ideal}')$.
	...
	we have $(d', z')$ is $O(\varepsilon)$-computationally\hannote{negl(lambda)?} indistinguishable to $(\Rej, \bot)=(d, z)$.
	...
	which in turn is $\negl(\lambda)$-computationally indistinguishable to $(d', z_{ideal}')$.\hannote{switch errors?}
	Since $\abs{d-d'}\leq O(\eps)$ \hannote{?},
...
\begin{lemma}[partition lemma; revision of Lemma 3.5 of \cite{arXiv:ChiaChungYam19}\footnote{$G_{0}$ and $G_{1}$ of this version are created from doing $G$ of \cite{arXiv:ChiaChungYam19} and post-select on the $ph,th,in$ register being $0^t01$ or $0^t11$ then discard $ph,th,in$. Property~\ref{property:partition-err} corresponds to Property~1. Property~\ref{property:partition-testing} corresponds to Property~4, with $2^{m-1}$ changes to $m-1$ because we only have $m$ possible choices of $\{c\}$. Property~\ref{property:partition-binding} corresponds to Property~5. Property~\ref{property-partition-norm-sum} comes from the fact that $G_0$ and $G_1$ are post-selections of orthogonal results of the same $G$.}]\label{lem:partition2}
	...
	For all $i\in[m]$, $\gamma \in \L\{\frac{\gamma_0}{T},\frac{2\gamma_0}{T},\dots,\frac{T\gamma_0}{T}\R\}$, there exist two efficient quantum circuit with post selection\footnote{A quantum circuit with post selection is composed of unitary gates followed by a post selection on some measurement outcome on ancilla qubits, so it produces a subnormalized state, where the amplitude square of the output state is the probability of post selection.} $G_{0,i,\gamma}$ and $G_{1,i,\gamma}$ such that for all (possibly sub-normalized) $\poly(\lambda)$-qubits quantum state $\ket{\psi}_{\regX,\regZ}$,  
...
\begin{proof}
	...
	By ~\cref{property:partition-binding} of ~\Cref{lem:partition2}, the above strategy is accepted with probability $1-\negl(n)$ when the prover didn't abort.
