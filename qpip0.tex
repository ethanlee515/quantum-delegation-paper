...
\Ethan{The following discussion is from Han-Hsuan}
...
This is good enough for our application, since we will feed the $XZ$ measurement results into the verification of $\QPIP_1$ protocol to ensure that the prover pick the history state as $\rho$ as we will show next.
...
\begin{enumerate}
	...
	\item The prover generates a classical string $a\leftarrow\cPMeasure{4}(c, \sigma)$ and sends it to back to the verifier.
	\item If it's a testing round ($c=0$), then the verifier generates and outputs $o\leftarrow\cVMeasure{T}(pk, y, a)$, where $o\in\set{\Acc, \Rej}$.
		If it's a Hadamard round ($c=1$), then the verifier computes $M_{XZ}(\rho, h)$ by $v\leftarrow\cVMeasure{H}(sk, h, y, a)$. It outputs $v$.
\end{enumerate}
\end{protocol}

Notice that the verifier doesn't in fact receive a measurement result on a testing round.
Instead, it exists to achieve a ``binding" property that gives certain guarantees against cheating provers.
...
\Ethan{Again change all n to lambda for sec. param. and fix variable labels}

\begin{lemma}\label{lem:urmila-binding}[binding property of Protocol~\ref{proto:urmila4}\cite{FOCS:Mahadev18a}]
If a prover's strategy in $\PiMeasure$ \hannote{need clarification} results in passing the testing round with probability $1-\negl(n)$, then there must exist a $\rho$ such that for all $h$, the verifier will produce a $z$ on the Hadamard round that is $\negl(n)$-computationally indistinguishable\footnote{Recall that quantum computational indistinguishability is defined in Definition~\ref{def:comp-indis}.} from $M_{XZ}(\rho,h)$, unless LWE can be efficiently solved by a quantum computer. 
\end{lemma}

We also mention a fact that intuitively captures the completeness of $\PiMeasure$.

\begin{fact}[\cite{FOCS:Mahadev18a}]~\label{lem:trivial-4-round-strategy}
...
We now use $\PiMeasure$ to modify our $\QPIP_1$ Protocol for $\SampBQP$, $\PiSamp=(\PSamp, \VSamp)$, to achieve a corresponding $\QPIP_0$ protocol $\PiNaive$.
Recall that $\PiSamp$ is a one-message protocol, where $\VSamp$ applies $X$ and $Z$ measurements non-adaptively to the incoming message.
We modify $\VSamp$ to pick all its measurement ahead of time, and measure the incoming message from $\PSamp$ through $\PiMeasure$ instead as below.
...
\begin{protocol}{Intermediate $\QPIP_0$ protocol $\PiNaive$ for a $\SampBQP$ problem $(D_x)_{x\in\set{0, 1}^*}$}
	...
	Protocol:
	\begin{enumerate}
		\item The verifier choose a $XZ$-measurement $h\leftarrow\cVNaive{1}(1^\lambda, 1^\varepsilon, x)$ from the distribution specified in Step~\ref{step:qpip1-verify} of $\PiSamp$.
		It picks between testing and Hadamard round by sampling $c\xleftarrow{\$}\zo$ uniformly at random.
	\item The prover prepares $\sigma\leftarrow\cPNaive{2}(1^\lambda, 1^\varepsilon, x)$ by running Step~\ref{step:qpip1-state-gen} of $\PiSamp$.
		\item \label{step:urmila-in-naive} The verifier and prover run $(\PMeasure(\sigma), \VMeasure(h))(1^\lambda, 1^{1/\varepsilon}, x)$.
	\item If it's a testing round, the verifier accepts or rejects based on the outputs of $\PiMeasure$.
		If it's a Hadamard round, the verifier obtains $v$. It finishes the verification step of Protocol~\ref{ProtoQPIP1} by using $v$ as the measurement result he would get and outputs $(d, z)$.
	\end{enumerate}
\end{protocol}

$\PiNaive$ has no soundness guarantee, but it has the following binding property that would be useful in proving the soundness of the actual protocol in Section~\ref{sec:qpip0}.
