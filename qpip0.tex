...
Composing it with $\QPIP_1$ protocol for $\BQP$ in \cite{mf16} results in a $\QPIP_0$ protocol for $\BQP$.
Now we try to compose $\PiMeasure$ similarly with our $\QPIP_1$ protocol $\PiSamp$ for $\SampBQP$ in order to obtain a $\QPIP_0$ protocol for $\SampBQP$ 

A direct composition $\PiSamp$ with $\PiMeasure$, however, results in $\PiNaive$ which does not provide reasonable completeness or accuracy guarantees.
...
To overcome this challenge, we run many copies of $\PiNaive$ in parallel so the verifier can generate its outputs from some copy.
We spend the majority of this section showing the soundness of this parallel repetition.
...
on the other hand, in a Hadamard round the verifier loses the opportunity \Ethan{focus on what the round can do, not what they can't} to catch cheating provers.
...
if a cheating prover $\PNaiveStar$, conditioned on it doesn't ``give up" \Ethan{Ambiguous. also need to associate this with $y=\bot$ if we haven't yet.} on the second message \Ethan{Move this clause to the end and say ``whenever it doesn't abort"}, passes the testing round with overwhelming probability,
...
\begin{theorem}[binding property of $\PiNaive$]
	...
	\Ethan{Write the conditional probability in terms of equations}
\end{theorem}
\begin{proof}
	...
	Now we construct another cheating $\BQP$ prover $\Pstar$ that is perfect \Ethan{``almost perfect"?} and non-aborting as follows.
...
\Ethan{Maybe say the $1/m$ thing here?}
...
Clearly \Ethan{If it's clear then just say why. Or say ``by inspection..."} $\PiSampZ$ is a $\QPIP_0$ protocol for $\SampBQP$ with negligible completeness error.
We now show that it is computationally sound. \Ethan{Lacking intuition currently...}
We first use the following partition lemma from \cite{arXiv:ChiaChungYam19}, which intuitively says that for each $i\in[m]$, there exist two efficient ``projectors" \footnote{Actually they are not projectors, but for the simplicity of this discussion let's assume they are.} $G_{0,i}$ and $G_{1,i}$ in the prover's internal space with $G_{0,i}+G_{1,i} \approx Id$. $G_{0,i}$ represents the subspace that are rejected with high probability \Ethan{change the characterization to whether the prover ``knows"...} in the testing round of $i$-th copy, while $G_{1,i}$  represents the subspace accepted with high probability\footnote{This is also not technically correct as we will discuss later.} in the testing round of $i$-th copy.
\Ethan{Maybe technical overview goes here. That ``tree or something"...}
\Ethan{TODO then talk about why other naive approaches don't work: stuff doesn't commute and whatnot}
...
\begin{lemma}[partition lemma; revision of Lemma 3.5 of \cite{arXiv:ChiaChungYam19}\footnote{$G_{0}$ and $G_{1}$ of this version are created from doing $G$ of \cite{arXiv:ChiaChungYam19} and post-select on the $ph,th,in$ register being $0^t01$ or $0^t11$ then discard $ph,th,in$. Property~\ref{property:partition-err} corresponds to Property~1. Property~\ref{property:partition-testing} corresponds to Property~4, with $2^{m-1}$ changes to $m-1$ because we only have $m$ possible choices of $\{c\}$. Property~\ref{property:partition-binding} corresponds to Property~5. Property~\ref{property-partition-norm-sum} comes from the fact that $G_0$ and $G_1$ are post-selections of orthogonal results of the same $G$.}]\label{lem:partition2}
	Let $\lambda$ be the security parameter and  $\eps$ be the accuracy parameter \Ethan{Is this the global one?}. Let $(U_0,U)$ be a prover's strategy in a $m$-fold parallel repetition of $\PiMeasure$, where $U_0$ is how the prover generates $\vec{y}$ on the second message, and $U$ is how the prover generates $\vec{a}$ on the fourth message. Denote the string $0^{i-1}10^{m-i} \in \zo^m $ as $e_i$, which corresponds to Hadamard round on the $i$-th copy and testing round on all others. Let $\gamma_0 \in[0,1]$, and $T\in \mathbb{N}$ such that $\gamma_0=\poly(\eps)$ and $T=1/\poly(\eps)$.
...
\Ethan{The following is still too quick. Don't use all properties up front in short succession.}
...
For all $k\leq m$, $d\in \zo^k$, $\gamma\in\set{\frac{\gamma_0}{T},\frac{2\gamma_0}{T},\dots,\frac{T\gamma_0}{T}}$, and $\ket{\psi} \in H_{\regX,\regZ}$, define \Ethan{This isn't correct; $\gamma$ should be independent for each copy}
$$\ket{\psi_{d,\gamma}}\defeq G_{d_k,k,\gamma}\ldots G_{d_2,2,\gamma} G_{d_1,1,\gamma} \ket{\psi}.$$
For example \Ethan{``take $k=3$"...}, $\ket{\psi_{110, \gamma}}$ intuitively corresponds to the component that would pass the first two copies' testing rounds and fail the third one.
\Ethan{Comment about $\ket{\psi_{good}}$ or whatever it is}
...
Before doing so, we present a couple technical lemma.
