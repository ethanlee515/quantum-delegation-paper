...
\begin{protocol}{Intermediate $\QPIP_0$ protocol $\PiNaive$ for a $\SampBQP$ problem $(D_x)_{x\in\set{0, 1}^*}$}
	...
	\begin{enumerate}
		...
		\item \label{step:urmila-in-naive} The verifier and prover run $(\PMeasure(\rho), \VMeasure(h))(1^\lambda)$,
			where on $\PiMeasure$'s fourth message, the prover can instead choose to abort by sending $a=\bot$.
			This is useless for the cheating prover but useful for proving one of our theorems later. \Ethan{Where/how to say this?}
...
\begin{lemma}\label{lem:naive-qpip0-binding}[binding property of $\PiNaive$]
	...
	then the verifier's output in the Hadamard round is $(\varepsilon+\negl(\lambda))$-computationally indistinguishable from $(d, z_{ideal})$.
\end{lemma}
...
\begin{proof}
	...
	$\cVVer(M_{XZ}(\rho, h))\approx_{c, \varepsilon}(d, z_{ideal})$.
	So we conclude that
	$(d, z)\approx_{c, \varepsilon+\negl(\lambda)}(d, z_{ideal})$.
\end{proof}
