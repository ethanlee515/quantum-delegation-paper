...
%    \item Verifier start with description of quantum circuit $C$, input string $x$ 
    ...
    \item Run $N=O(1/\varepsilon)$ copies of protocol~\ref{proto:urmila4} in parallel, with following modifications. On the second round, for each copy, the prover choose the state specified by Protocol~\ref{ProtoQPIP1} as $\rho$. On the third round, the verifier uniformly randomly pick an $i\in [N]$, do a Hadamard round on $i$-th copy, and has do test rounds on all other copies.
    \item \label{step:multi-testing}Verifier rejects if the results on any of the test rounds is wrong. 
    \item Verifier run the verification procedure of Protocol~\ref{ProtoQPIP1} on the string $z$ decoded from the Hadamard round.
\end{enumerate}
\end{protocol}
...
\begin{proof}
 ...
 [[partition lemma -> Parallel repetition]], except for probability $\eps$, either the verifier reject at step~\ref{step:multi-testing} or $z$ has binding property of Theorem~\ref{thm:urmila-binding}. 
...
\end{theorem}
\begin{proof}
We run Mahadev's 4-round protocol \ref{} and use parallel repetition to reduce the soundness error. \hannote{describe $N$ time para rep}. 
\end{proof}
