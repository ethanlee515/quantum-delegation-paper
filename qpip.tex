...
\begin{prf}
	...
	Now we separate the $\rho_j$ into two categories: $J=\set{j\in I:p_j=\cV_\GS(\rho_j)<\frac{1}{2}-2h}$ and its complement. \Ethan{TODO import and use the right ``given" command.}
	...
	$$P[\cV_\GS(\phi)=acc]>\frac{1}{2}-2\varepsilon\Rightarrow\braket{\phi|H_{C'(x)}|\phi}<\frac{1}{T^\lambda}$$
	we then have
