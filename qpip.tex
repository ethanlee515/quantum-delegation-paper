...
\begin{protocol}{$\QPIP_1$ protocol $\PiSamp=(\PSamp, \VSamp)$ for the $\SampBQP$ instance $(D_x)_{x\in\set{0, 1}^*}$}
	...
	\begin{enumerate}
		...
		\end{enumerate}
			Let $Z=\sum_{i\in I} z_i$. If $Z>\frac{m}{2}-\kappa m$ where $\kappa=\frac{c\varepsilon^2}{2\left(T+\frac{T}{\varepsilon}\right)^5}$ and $c\in\bbR$ as in \Cref{cor:HamCheck} then the verifier accepts and outputs $y$. Otherwise, it rejects. \hannote{... outputs $(acc, y)$. Outputs $(rej,\bot)$}
	\end{enumerate}
\end{protocol}
...
\begin{thm}
    \label{QPIP1thm}
	$\PiSamp$ has negligible completeness error and $O(\varepsilon)$ soundness error. \hannote{remind completeness and soundness with a footnote.}
\end{thm}
\begin{prf}
