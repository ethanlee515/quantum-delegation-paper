...
\begin{protocol}{$\QPIP_1$ protocol $\PiSamp=(\PSamp, \VSamp)$ for the $\SampBQP$ instance $(D_x)_{x\in\set{0, 1}^*}$}
	...
	\begin{itemize}
		...
		\item Let $H_{C'(x)}=\sum_i \alpha_i H_i$ be the local Hamiltonian instance associated with the computation of $C'(x)$. \Ethan{Refer to the appropriate previous theorem}
	...
	Protocol:
	\begin{enumerate}
		\item\label{step:qpip1-state-gen} The honest prover prepares $M=505\frac{T^{40}\lambda^2}{\varepsilon^{48}}$ copies \Ethan{$505=2\times 7\times 6\times 6+1$} of $\ket{\psi_{C'(x)}}$ and sends all of it to the verifier qubit-by-qubit.
...
\begin{prf}
	...
	\fi
	By \Cref{HamCheckClose} we have
	$$\norm{\tau_j - \ket{\psi_{C'(x)}}\bra{\psi_{C'(x)}}}_{tr}\leq\frac{2}{\sqrt{3}}\sqrt{2\kappa\cdot2\sum_i\abs{\alpha_i}}=\frac{\varepsilon}{6}.$$
