...
\end{proof}
\hannote{something. pull protocol out?}
\begin{theorem}
    \label{QPIP1thm}
	For any $L\in\SampBQP$,  there exists a $\QPIP_1$ protocol for $L$ with negligible completeness and soundness $(O(T^{-c}), O(T^{-c}))$ for any constant $c$. Furthermore, in this protocol, the verifier doesn't need to apply any gates, and only needs to apply $X$ and $Z$ measurements.
\end{theorem}
\begin{proof}
...
Here we combine \autoref{QPIP1thm} with results from \cite{mahadev_delegation} to create a delegation protocol for $\SampBQP$ for fully classical clients.
...
Let $\varepsilon>0$. Pick $N$ large enough so that by geometric probability getting a 
