...
Note that $\VSamp$ \Ethan{TODO Define this} only needs to apply $X$ and $Z$ measurements, and is classical otherwise.
...
\begin{prf}
	...
	\Ethan{Think about notation... maybe $s$? typeof($A$) is strange. Also an error here.}
	That is, \Ethan{type out ``there exists" if in sentence} $\exists\rho=\sum_j w_j\rho_j^{\otimes m+1}$
	...
	Now we separate the $\rho_j$ into two categories base on $p_j$, the accept probability of $\cVGS(\rho_j)$: \Ethan{Wording issue; make it obvious I'm defining $p_j$}
	$$J=\set{j\in\bbN|p_j<\frac{1}{2}-2\kappa}$$
	
	\Ethan{Definition of $J$ is extraneous; just cut into two cases.}
	...
	\Ethan{Delete the following and just say it's standard application of Chernoff bound}
	...
	so the data register has $1-\poly(\varepsilon)$ probability to be $C(x)$ at this point. \Ethan{get rid of the remaining polys}
