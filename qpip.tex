...
\begin{protocol}{$\QPIP_1$ protocol $\PiSamp$ for computation of $C(x)$ and error parameter $\varepsilon$}\label{ProtoQPIP1}
	...
	Let $\ket{\psi_{C'(x)}}$ be the local Hamiltonian ground state associated with $C'$ as defined in \Cref{thm:LHReduction}, and
	$\cVGS$ be the way to check this ground state as defined in \Cref{AlgGroundStateCheck}.
	...
	\begin{enumerate}
		\item The honest prover prepares $M=\varepsilon^{-8}\ln(\frac{T}{\varepsilon})$ copies \Ethan{That isn't an integer...?} of $\ket{\psi_{C'(x)}}$ and sends all of it to the verifier qubit-by-qubit.
