...
\begin{protocol}{$\QPIP_1$ protocol $\PiSamp=(\PSamp, \VSamp)$ for the $\SampBQP$ instance $(D_x)_{x\in\set{0, 1}^*}$}
	...
	\begin{itemize}
		...
		\item \XW{Let $s\leq 3T+\frac{6T}{\varepsilon}$ be the number of qubits in $\ket{\psi_{C'(x)}}$ Why this number?} \Ethan{This is used in de Finetti theorem. Btw I forgot to count ancilla, so it wasn't correct before.}
