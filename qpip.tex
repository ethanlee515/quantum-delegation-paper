...
\begin{protocol}{$\QPIP_1$ protocol $\PiSamp=(\PSamp, \VSamp)$ for $\SampBQP$}
	\label{ProtoQPIP1}

	Inputs:
	\begin{itemize}
		\item $\SampBQP$ instance $(D_x)_{x\in\set{0, 1}^*}$
	...
	Ingrediants:
	\begin{itemize}
		\item Quantum circuit $C$ of size $T$ consisting of only Hadamard and Toffoli gates \Ethan{Something about $\varepsilon_0$?}
	...
	\begin{enumerate}
		...
		\end{enumerate}
			Let $Z=\sum_{i\in I} z_i$. If $Z>\frac{m}{2}-\kappa m$ where $\kappa=\frac{c\varepsilon^2}{2\left(T+\frac{T}{\varepsilon}\right)^5}$ and $c\in\bbR$ as in \Cref{cor:HamCheck} then the verifier accepts and outputs $y$. Otherwise, it rejects.
	\end{enumerate}
\end{protocol}
...
\end{prf}
\begin{cor}
	By choosing $\tilde{\varepsilon}=O(\varepsilon)$ accordingly, $\PiSamp$ has $\tilde\varepsilon$ soundness error. In which case, it runs in $\poly(T, \tilde\varepsilon^{-1})$ time, and has negligible completeness errors.
\end{cor}
