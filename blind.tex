...
For the sake of simplicity, we omit the security and accuracy parameters.
...
\Ethan{Say in words I'm going to compile proto 5 (which is a general form of...) to proto 6}

\Ethan{Add lambda back}

\begin{protocol}{$\QPIP_0$ protocol $\Pi_0=(P, V)(x)$ where only the verifier receives outputs}
	
	Public input: $x$ \Ethan{Add footnote here about eps}
	\begin{enumerate}
...
\begin{protocol}{Blind $\QPIP_0$ protocol $\Piblind=(\Pblind, \Vblind(x))$ correponding to $\Pi_0$}
    \Ethan{Sync common or public inputs wording. Prefer common.}
	Verifier's input: $x$

	Let $L=\poly(x)$ \Ethan{Quantify/define L more clearly}, $\lambda=\poly(x)$.
...
\begin{proof}
	...
	By a standard hybrid argument, we observe that $\Hyb_1$ is computationally indistinguishable with $\Hyb_{T+1}$.
	Furthermore, $\Hyb_1$ is blind since the prover isn't provided any information about any secret keys.
	\Ethan{do hybrid on 0 too and make sure it fits the dfns. Also should mention why hybriding works}
