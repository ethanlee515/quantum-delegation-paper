...
We write $\norm{\rho_1-\rho_2}_{tr}$ for the trace distance between $\rho_1$ and $\rho_2$. For all $n$-qubit states $\rho_1, \rho_2\in\cB^{\otimes n}$ we have $\norm{\rho_1-\rho_2}_{tr}\leq\sqrt{1-F(\rho_1, \rho_2)}$.
...
\Ethan{Look up ``interactive protocols for BQP"}
\begin{definition}
	We say $\Pi=(P, V)$ is a protocol for the langauge $L$ if for all $x\in\zo^*$: TODO
\end{definition}

\begin{definition}
	Let $\Pi=(P, V)$ be a protocol with security parameter $\lambda$ and accuracy parameter $\varepsilon=\frac{1}{\poly(\lambda)}$.
	We say $\Pi$ is a protocol for the sampling problem $(D_x)_{x\in\zo^*}$ with completeness error $c$ if for all $x\in\zo^*$:
	\begin{itemize}
		\item At the end of the protocol, $V$ outputs $(d, z)$ where $d\in\set{\Acc, \Rej}$.
		\item Let $(d, z)\leftarrow(P, V)(x)$. Then $d=rej$ with probability at most $c(\lambda)$.
		\item For all cheating prover $P^*$, let $(d, z)\leftarrow(P^*, V)(x)$.
			Define $z_{ideal}$ by
			$$\begin{cases}
				z_{ideal}=\bot & \text{if } d=\Acc\\
				z_{ideal}\leftarrow D_x & \text{if } d=\Rej
			\end{cases}$$\hannote{swap rej and acc}
			then $\norm{(d, z) - (d, z_{ideal})} \leq \varepsilon$ \Ethan{make stat. distance notation consistent}.
	\end{itemize} \hannote{need computational version of soundness?} \Ethan{The norm at the end? Nah it's statistical distance}
\end{definition}
...
\begin{definition}
	...
	Then it is \emph{blind} if...

	\Ethan{TODO something about views}

	\Ethan{See https://www.iis.sinica.edu.tw/~kmchung/download/ConstCZK.pdf}
\end{definition}
...
\Ethan{State the one below; prove the one above in appendix}
