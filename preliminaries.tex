...
\begin{definition}
	...
	\begin{itemize}
		\item If $x\in L$, then $\Pr\left[(P, V)(x)=\Rej\right]<c(\abs{x})$.
		\item If $x\notin L$, then for all cheating prover $P^*$, $\Pr\left[(P^*, V)(x)=\Acc\right]<s(\abs{x})$.
	\end{itemize}
\end{definition}
...
\begin{definition}
	...
	\end{itemize} \hannote{need computational version of soundness?} \Ethan{Yeah. Will fix.}
\end{definition}
