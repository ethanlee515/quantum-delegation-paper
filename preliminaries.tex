...
We use the projection lemma from \cite{kempe_kitaev_regev_2006}, which states the condition under which one can estimate the ground state energy of $H_1 + H_2$ with that of $H_1\big|_{\ker H_2}$.
...
We also use the quantum fully homomorphic encryption scheme given in \cite{mahadev_qfhe} which is compatible with our use of a classical client.
...
\begin{thm}
	...
	\begin{itemize}
		...
		\item \Ethan{Is this obvious from the construction or do we need proof here?} Let $\Lambda$ be a product of $X$ and $Z$ measurements. Then there exists $\Lambda'$ product of $X$ and $Z$ measurements and $\widehat{\mathsf{QHE.Dec}}$ so that
