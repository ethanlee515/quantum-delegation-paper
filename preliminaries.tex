...
\begin{itemize}
	...
	\item Its ciphertext takes the form $(X^xZ^z\ket\rho Z^zX^x, c_{x, z})$, where $\rho$ is the plaintext and $c_{x, z}$ is a ciphertext that decodes to $(x, z)$ under a certain classical homomorphic encryption scheme.
	We assume without loss of generality that the length of $c_{x, z}$ is only dependent on the length of $\ket\phi$ by padding $0$s if necessary.
\end{itemize}
