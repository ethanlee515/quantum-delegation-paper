...
\begin{dfn}
	...
	Let $(\bbP, \bbV)(x)$ be an interactive protocol with the verifier's initial input being $x$.
	Then it is IND-CPA secure if any polynomial time adversary $\cA$ can't win \myprotoref{protoIndCPA} with probability better than $\frac{1}{2}+\negl(\lambda)$ \Ethan{Check if it's actually lambda}
\end{dfn}
...
\begin{rmk}
	\Ethan{Maybe something about this being (hopefully) equivalent to simulator-based security?}
\end{rmk}
