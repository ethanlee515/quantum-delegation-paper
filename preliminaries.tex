...
\begin{definition}
	...
	\begin{itemize}
		\item On public inputs $1^\lambda$, $1^{1/\varepsilon}$, and $x\in\zo^{\poly(\lambda)}$, $V$ outputs $(d, z)$ where $d\in\set{\Acc, \Rej}$.
			If $d=\Acc$ then $z\in\zo^{m(\abs{x})}$ where $m$ is given in \Cref{dfn:sampling-problem} \Ethan{Like this, right?}, otherwise $z=\bot$.
		\item (Completeness):
...
\label{def:comp-indis}
    \Ethan{TODO Mention asymptotic/ensemble. Things should be in terms of $\lambda$. Should also move to other subsections instead, since this is computatational. Maybe move to end of 2.3.}
	...
	That is, for all quantum polynomial-time \hannote{formalize} distinguishers $\mathsf{D}$,
