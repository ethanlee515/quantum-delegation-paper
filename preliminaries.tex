...
\begin{definition}
	A sampling problem $S=(D_x)$ is said to be in $\QPIP_{XZ}$ with completeness $c$ and soundness $s$ if there exists a pair of algorithms $(\bbP, \bbV)$ with the following properties with input $x$:
	TODO Fix/redefine this to allow $\bbV$ a single qubit and ability to ONLY measure in X/Z
	\begin{itemize}
		\item $\bbP$ is a hybrid quantum-classical machine. Its classical part is a $BPP$ machine. Its quantum part is a register of a single qubit, on which the verifier can apply $X$ or $Z$ measurements.
