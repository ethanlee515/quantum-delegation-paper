...
We extend computational indistinguishability to subnormalized states by taking the distinguisher $\mathsf{D}$ to output subnormalized outputs on subnormalized inputs.
We also extend it to distributions, which can be treated as mixed states.
...
\begin{lem}[triangle inequality for computational distance] \label{lem:computational-triangle0}
    ...
    \begin{align}
		D_1\approx_{c, \varepsilon}D_2\\
		D_2\approx_{c, \varepsilon'}D_3.
    \end{align}
    We have 
    \begin{align}
		D_1\approx_{c, \varepsilon+\varepsilon'}D_3.
    \end{align}
\end{lem}
