...
We define $\SampBQP$ the same way as from \cite{aaronson_2013}.
...
\begin{definition}
	$\SampBQP$ is the class of sampling problems $S=(D_x)$ for which there exists a polynomial-time quantum algorithm that, given $x$ as input, samples from from a probability distribution $C_x$ such that $\norm{C_x-D_x}<\varepsilon$.
\end{definition}
