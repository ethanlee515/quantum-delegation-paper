...
We define a few relevant complexity classes. \Ethan{Do we need to define $\BPP$ and $\BQP$ too?}

\begin{dfn}
	$\QMA$ is the quantum analogue of the class $\MA$. The verifier in $\QMA$ is now a $\BQP$ machine.
\end{dfn}

\begin{dfn}
	$\FBQP$ is the functional analogue of the class $\BQP$. That is, every instance corresponds to a string rather than a yes/no bit. The error probability is still bounded above away from $\fot$ for some uniform quantum circuit family.
\end{dfn}
