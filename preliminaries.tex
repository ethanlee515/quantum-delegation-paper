...
\begin{definition}
	...
	Let $\Pi=(P, V)$ be a $\QPIP_\tau$ protocol.
	We say it is a protocol for the $\SampBQP$ instance $(D_x)_{x\in\zo^*}$ with completeness error $c$ and statistical (resp., computational) soundness if the following holds:
	\begin{itemize}
...
In the context of blind protocols, the above definitions should be modified so that $x$ is the verifier's private input.
We define blindness with an accuracy parameter $\varepsilon$, which exists in protocols for $\SampBQP$ instances;
one can leave it out where appropriate, such as in protocols for $\BQP$ languages.

\begin{definition}
	Let $\Pi=(P, V)$ be an interactive protocol with common inputs $1^\lambda$ and $1^{1/\varepsilon}$, and verifier's private input $x\in\zo^*$.
	We say $\Pi$ is \emph{blind} if for all cheating prover $P^*$ the following ensembles are computationally indistinguishable over $\lambda$:
	\begin{itemize}
...
\begin{definition}
	...
	\begin{itemize}
		\item \emph{Key generation.}
			The algorithm $(pk, sk)\leftarrow\QGen(1^\lambda)$ takes a unary representation of the security parameter and outputs a public key encryption key $pk$ and a secret decryption key $sk$.
		\item \emph{Encryption.}
			The algorithm $c\leftarrow\QEnc(pk, \mu)$ takes the public key $pk$ and a single bit message $\mu\in\zo$ and outputs a ciphertext $c$.
		\item \emph{Decryption.}
			The algorithm $\mu^*\leftarrow\QDec(sk, c)$ takes the secret key $sk$ and a ciphertext $c$ and outputs a message $\mu^*\in\zo$.
			It must be the case that
				$$\QDec(sk, \QEnc(pk, \mu))=\mu$$
			with overwhelming probability in $\lambda$.
		\item \emph{Homomorphic Evaluation.}
			The algorithm $c_f\leftarrow\QEval(pk, f, c_1, \ldots, c_l)$ takes the public key $pk$, a quantum circuit \Ethan{?} $f$, and a set of $l$ ciphertexts $c_1,\cdots,c_l$ and outputs a ciphertext $c_f$.
			It must be the case that
...
\begin{definition}[Compactness and Full Homomorphism]
	\Ethan{This is copied from Mahadev. Not sure how much of this we need. We need at least how leveled HE is defined.}
	...
	and can only evaluate depth $L$ Boolean circuits where $L$ is polynomial in $\lambda$. A scheme is quantum pure or
...
\begin{definition}
	\Ethan{This was copied from Mahadev. Not sure where $L$ (level) goes.}
