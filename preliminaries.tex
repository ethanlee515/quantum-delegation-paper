...
\Ethan{Use Mahadev's definitions, but add eps param

change to subnormalized states

Need to define what it means to apply algorithm to subnormalized states

Two families of (potentially sub-normalized) states... leave author note for applying algorithms to subnormalized states; might be ok

change ``attacker" to ``distinguisher"

sec. param change from n to lambda}

\Ethan{Comment out entire section including triangle ineq.; just leave what's said above}
...
\begin{definition}
	...
	\begin{itemize}
		\item On public inputs $1^\lambda$, $1^{1/\varepsilon}$, and $x\in\zo^{\poly(\lambda)}$, $V$ outputs $(d, z)$ where $d\in\set{\Acc, \Rej}$. If $d=\Acc$ then $z$ is in the domain \Ethan{?} of $D_x$, otherwise $z=\bot$.
		\item (Completeness):
