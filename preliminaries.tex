...
\Ethan{After this point, add more remarks in this section}
...
\Ethan{``Streaming" model; can send qubits one-by-one to let client measure many qubits this way.

Tone: Review/introduce. It's not new. Use Urmila's use case as example.}
...
\Ethan{Need discussion for dfn. 2.7.
Currently there's a remark after about not needing soundness error.
Discuss more.

Difference between SampBQP and BQP definitions.
In particular, SampBQP has additional output (not 0/1).

Two sources of error. Kinda like ``DI" device independent/QKD?
Condition on acc...
Really depends on target audience. If not crypto...

Maybe just remark that there are 2 params but this definition captures both

Again, it's different from BQP so explain why/where.
Make it more friendly to people who aren't familiar with this approach.

Also discuss whether soundness error needs additional param. besides inherited from SampBQP}
...
\Ethan{The following is more crypto based

Others don't have a fully general definition,
or it's just statistical/OTP.

Need explanation/discussion for this.
Nobody else writes it like this,
but this is the correct way to write it.

Explain what is this saying.}
...
\Ethan{Maybe swap 2.9 and 2.8 or move it up even more.
If it's before 2.7 then we can just write 2.7 in terms of this.

Consider making this its own section.}
...
\Ethan{Chernoff bound looks like it's in the middle of nowhere. Just get rid of it.}
...
\Ethan{This section should be fine; just leave it alone.
It's just technicalities copied from another paper.}
