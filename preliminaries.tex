...
\begin{definition}
	...
	\begin{itemize}
		\item On public inputs $1^\lambda$, $1^{1/\varepsilon}$, and $x\in\zo^{\poly(\lambda)}$, $V$ outputs $(d, z)$ where $d\in\set{\Acc, \Rej}$. If $d=\Acc$ then $z\in\zo^{TODO}$ \Ethan{TODO}, otherwise $z=\bot$.
		\item (Completeness):
