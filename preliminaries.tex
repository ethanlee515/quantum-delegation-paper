...
\begin{definition}
	We say $\Pi=(P, V)$ is a protocol for the langauge $L$ with completeness error $c$ and soundness error $s$ if for all $x\in\zo^*$:
	\Ethan{TODO: errors are functions of input length}
	\begin{itemize}
		\item If $x\in L$
			$\Pr\left[(P, V)(x)=(1^n, x)\right]$
		\item TODO SOUNDNESS
	\end{itemize}
\end{definition}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item Let $(d, z)\leftarrow(P, V)(x)$. \Ethan{Same problem as last definition...} Then $d=rej$ with probability at most $c(\lambda)$.
	...
	\end{itemize} \hannote{need computational?} \Ethan{Yeah. Will fix.}
\end{definition}
...
\begin{definition}
	...
	\end{itemize}
	\Ethan{Do I need to write out unrelated inputs to be fully general?}
