...
\begin{protocol}{Blind $\QPIP_1$ 1-message protocol $\Piblind = (\Pblind, \Vblind)$ for $\BQP$ language $L$}
	...
	\begin{enumerate}
		...
		\item The prover homomorphically evaluates $\cPMF{1}$ on input $ct_x$, generating $\sigma\leftarrow\cPblind{1}(pk, ct_x)=\QEval_{pk}(\cPMF{1}, ct_x)$.
...
\begin{prf}
	...
	It then sends $ct_x$ and $pk$ to $\Pblindstar$ \Ethan{Just run it, don't send it} and gets $\sigma$ back.
	Finally, it decrypts
	...
	Observe the experiments $A$ and $B$ \Ethan{TODO} are identical except...
...
\begin{protocol}{Blind $\QPIP_0$ protocol $\Pibz$ for $BQP$ language $L$}
	...
	\begin{enumerate}
		\item The verifier generates $\Lambda, \Lambda', pk, sk, ct_x$ the same way as in \cref{proto:BlindBQP}. \Ethan{Need more notation? But how?}
