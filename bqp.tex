...
\begin{thm}
	...
	for all $\Lambda=\Lambda_1\otimes\ldots\otimes\Lambda_n$ where each $\Lambda_i$ is a single-qubit $X$ or $Z$ measurement: \Ethan{Just use basis choice string instead}
	$$\cM_\QHE(\Lambda)=\Lambda'=\Lambda'_1\otimes\ldots\otimes\Lambda'_m$$
	where each $\Lambda_i'$ is a single-qubit $X$ or $Z$ measurement, and \Ethan{Function compositions feels weird}
...
\begin{prf}
	...
	Hence we have $\PMFstar$ with noticeable acceptance probability $\varepsilon$, which is a contradiction. \Ethan {Write more details; more formula}
\end{prf}
...
\begin{thm}[from \cite{parallelrep, alagic2019twomessage}]
	\label{thm:MFZrep}
	$\PiMFZ$ can be repeated in parallel, obtaining $\PiMFZrep$ that has negligible completeness and soundness errors. \Ethan{Just remove this}
\end{thm}

\Ethan{Just define parallel repetition in words}
