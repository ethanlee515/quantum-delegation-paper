...
\begin{thm}
	...
	Then \protoref{proto:BQP} is a $\QPIP_1$ and constant round protocol for $\BQP$ with negligible completeness and soundness.
\end{thm}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\State The verifier runs $\mathsf{QHE.Keygen}(\lambda)$ to get $pk, sk$. \Ethan{Security param?}
		...
		\State The verifier decodes qubit-by-qubit \Ethan{hmm?}
		\State The verifier uses $n$ copies to check for $x\in L$, and the other $n$ copies to check for $x\in\overline{L}$
	\end{algorithmic}
\end{algorithm}
...
\begin{thm}
	...
	Then \protoref{proto:BlindBQP} is a $\QPIP_1$, constant round, and IND-CPA protocol for $\BQP$ with negligible completeness and soundness.
\end{thm}
\begin{proof}
