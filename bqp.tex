...
In this section we build our constant-round blind delegation protocols for $\BQP$ and $\FBQP$ with classical clients.
We first construct our protocol for $\BQP$ in \cref{subsec:BlindBQP}, then use it to construct our protocol for $\FBQP$ in \cref{subsec:BlindFBQP}.

\subsection{Blind Delegation for BQP}
\label{subsec:BlindBQP}

We build our constant-round blind delegation protocol for $\BQP$ first.
We start with the protocol for $\BQP$ from \cite{mf16}, where the verifier requires a single-qubit register and is classical otherwise.
We first obtain blindness by running said protocol under the homomorphic encryption given in \cite{mahadev_qfhe}.
We then make the verifier fully classical using the same argument as used in \cite{FOCS:Mahadev18a}.
Lastly, we use analysis from \cite{parallelrep, alagic2019twomessage} to ensure that it has negligible completeness and soundness errors while still being constant-round.
