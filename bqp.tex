...
\Ethan{Use only pk, sk and combine evk into pk}
...
The verifier samples a product measurement \Ethan{for consistency, ``measurement basis choice"} $\Lambda$ of $X$ and $Z$s according to a certain distribution, and applies it to $\rho$.
...
\begin{protocol}{1-message $\QPIP_1$ protocol $\PiMF = (\PMF, \VMF)$ for $\BQP$ language $L$}
	...
	\begin{enumerate}
		\item The prover generates a certificate state $\rho\leftarrow\cPMF{1}(x)$ and sends $\rho$ to the verifier qubit-by-qubit. \Ethan{Say it's $m$ qubits}
...
\begin{thm}
	...
	Let $n\in\bbN$.
	Let $m\in\bbN$ be the length of the ciphertexts under $\mathsf{QHE}$ corresponding to plaintexts of length $n$. \Ethan{$m$ isn't well-defined this way; missing security param. and depth param.}
...
\Ethan{We state our construction of blind QPIP 1 for BQP by compiling...}
...
\begin{protocol}{Blind $\QPIP_1$ 1-message protocol $\Piblind = (\Pblind, \Vblind)$ for $\BQP$ language $L$}
	...
	\begin{enumerate}
		...
		\item The prover homomorphically evaluates $\cPMF{1}$ on input $ct_x$, generating $\sigma\leftarrow\cPblind{1}(evk, ct_x)=\mathsf{QHE.Eval}_{evk}(\cPMF{1}, ct_x)$.
...
\begin{prf}
	...
	Suppose for the sake of contradiction \Ethan{First specify this is for soundness proof} that there exists some cheating prover $\Pblindstar$ with some noticeable acceptance probability $\varepsilon$.
	...
	It then sends $ct_x$ and $evk$ to $\Pblindstar$ \Ethan{Just run it, don't send it} and gets $\sigma$ back.
	Finally, it decrypts
	...
	Consider the experiment corresponding to $(\Vblind, \Pblindstar)$:
    ...
    \Ethan{See google docs; remove the displayed math above and say in words}
	...
	\Ethan{Here finish the proof by contradiction. Something about epsilon.}
\end{prf}
...
\Ethan{Is the following even necessary?}
...
\begin{thm}
	\cref{proto:repQPIP0BQP} has negligible completeness and soundness and is blind.
\end{thm}
\begin{prf}
