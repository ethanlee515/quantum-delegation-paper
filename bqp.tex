...
\begin{protocol}{Blind $\QPIP_1$ 1-message protocol $\Piblind = (\Pblind, \Vblind)$ for $\BQP$ language $L$}
	...
	\begin{enumerate}
		...
		\item The prover computes $\sigma\leftarrow\cPblind{1}$ by homomorphically evaluating $\cPMF{1}$ on input $ct_x$, generating $\sigma\leftarrow\QEval_{pk}(\cPMF{1}, ct_x)$.
...
\begin{prf}
	...
	Observe the experiments $A$ and $B$ \Ethan{TODO} are identical except how $w$ is obtained from $\sigma$.
	\Ethan{TODO Make more precise}
...
\Ethan{This sentence is from google docs so keep it around}
...
\end{thm}
\begin{prf}
	\Ethan{Don't open separate proof env}
