...
Note that in the above definition, there is an accuracy parameter $\eps$ and the quantum sampling algorithm only requires to output a sample that is $\eps$-close to the correct distribution in time $\poly(n,1/\eps)$, instead of requiring a perfect or negligibly close sample. \cite{aaronson_2013, Boson} discussed multiple reasons for allowing the inverse polynomial error, such as conceivable physical realizations of quantum computer may consist of inherent noise.
